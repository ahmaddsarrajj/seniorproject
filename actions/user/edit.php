...
if( $_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    do {
        if (empty($name) || empty($password) || empty($conf_password) || empty($role) || empty($city)) {
            $errorMesg = "All fields are required!";
            break;
        }
        if($password === $conf_password){
            $query = "UPDATE `user` SET `FNAME`='$fname',`LNAME`='$lname',`USERNAME`='$name',`EMAIL`='$email',`PASSWORD`='$password',`ROLE_ID`='$role',`CITY_ID`='$city' WHERE `SSN` = $id";
            $update   = mysqli_query($conn, $query);  
            $successMesg = "User Updated Correctly";
            header("location: ../../screen/dashboard/user.php");
            exit();
        }else{
...
if (isset($_SESSION['SSN']) && isset($_SESSION['USERNAME']) && isset($_SESSION['ROLE_ID']) && isset($_SESSION['CITY_ID']) == 1 ) {
?>
